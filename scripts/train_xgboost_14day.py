"""
Train XGBoost on 14-day features with temporal split and masked-row exclusion.

Outputs (all in models/, do NOT overwrite existing 12h model files):
  models/xgboost_14day.json
  models/threshold_14day.json
  models/metrics_14day.json
  models/evaluation_report_14day.md
  models/pr_curve_14day.png

Run from repo root: python scripts/train_xgboost_14day.py
"""
...
def main() -> None:
    ...
    ap.add_argument("--features", type=Path,
                    default=ROOT / "data" / "features_lagged_14day.parquet")
    ap.add_argument("--meta", type=Path,
                    default=ROOT / "data" / "features_lagged_14day_meta.json")
    ...
    n_pos = int(df["y"].sum())
    n_mask = int(df["mask"].sum()) if "mask" in df.columns else 0
    print(f"Loaded {len(df):,} rows  positives={n_pos}  masked={n_mask}")

    result = train_xgboost_14day(df, feature_names)
    model = result["model"]
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    json_path = args.out_dir / "xgboost_14day.json"
    model.save_model(str(json_path))
    print(f"Wrote {json_path}")

    # ── Threshold ─────────────────────────────────────────────────────────────
    thr_path = args.out_dir / "threshold_14day.json"
    thr_path.write_text(
        json.dumps({
    ...
    # ── Report ────────────────────────────────────────────────────────────────
    report_path = args.out_dir / "evaluation_report_14day.md"
    ...
    if result["y_val"].sum() > 0:
        ...
        ax.axvline(result["threshold_report_val"].get("tp", 0) /
                   max(result["y_val"].sum(), 1),
    ...
    ax.set_title("Precision–Recall (validation) — 14-day model")
    ax.legend()
    ax.grid(True, alpha=0.3)
    pr_png = args.out_dir / "pr_curve_14day.png"
    fig.tight_layout()
    ...
    metrics_path = args.out_dir / "metrics_14day.json"
